...
/**
 * Z.AI adds a visible and an invisible AI watermark by default. Accounts that
 * have completed Z.AI's official watermark-removal authorization can opt out
 * by setting ZAI_WATERMARK_ENABLED=false in their local .env file.
 */
export function getWatermarkEnabled(): boolean {
  const raw = process.env.ZAI_WATERMARK_ENABLED?.trim().toLowerCase();
  if (raw == null || raw === "") return true;
  if (["true", "1", "yes", "on"].includes(raw)) return true;
  if (["false", "0", "no", "off"].includes(raw)) return false;
  throw new Error("ZAI_WATERMARK_ENABLED must be true or false.");
}
