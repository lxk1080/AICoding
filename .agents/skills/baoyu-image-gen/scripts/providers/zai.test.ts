...
test("Z.AI watermark is enabled by default and can be disabled after account authorization", (t) => {
  useEnv(t, { ZAI_WATERMARK_ENABLED: null });
  assert.equal(getWatermarkEnabled(), true);

  process.env.ZAI_WATERMARK_ENABLED = "false";
  assert.equal(getWatermarkEnabled(), false);

  process.env.ZAI_WATERMARK_ENABLED = "invalid";
  assert.throws(() => getWatermarkEnabled(), /must be true or false/);
});
